...
let
  ...
  error-count-overlay = self: super: {
  ...
  error-count = pkgs.haskell.packages.${compiler}.callCabal2nix "error-count" (gitignore ignorance ./.) {};
...
in with pkgs; with lib;
  ...
  then pkgs.haskell.packages.${compiler}.shellFor {
    inherit withHoogle;
    packages    = _: [ error-count ];
    COMPILER    = compiler;
    buildInputs = ghcTools;
    shellHook   = ''
      cat ${./intro}
    '';
  } else chill error-count
